...
// an image of a cat dancing, keep it dancing for 10 seconds, and then replace
// the img with the original image and have it continue the walk.
//
// Bonus #4: Pretty much go nuts or whatever.

let walkingCat = document.getElementsByTagName('img')[0];
  walkingCat.style.left = '0px';

const theWalk = function() {
  const oldPos = parseInt(walkingCat.style.left);
  const newPos = oldPos + 10;
  walkingCat.style.left = newPos + 'px';
  if (walkingCat.style.left === '1200px') {
    window.clearInterval(catWalk);
  }
};

const catWalk = window.setInterval(theWalk, 50);
