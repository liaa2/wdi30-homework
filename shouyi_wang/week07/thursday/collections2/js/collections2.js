...
//Sort the people by 'uid'
let peopleSortedByUid = _.sortBy(people, "uid");
console.log(peopleSortedByUid);

//Group the random words by the lower case version of their first letter
let groupedWords = _.groupBy(words, function(word) {
  return word[0].toLowerCase();
});
console.log(groupedWords);

//Check to see if all the words have more than 3 characters
let allLongerThan3 = _.every(words, function(word) {
  return word.length > 3;
});

console.log("All longer than 3: " + allLongerThan3);

//Check if some words are over twenty characters long
let someLongerThan20 = _.some(words, function(word) {
  return word.length > 20;
});

console.log("Some longer than 20: " + someLongerThan20);

//Get an array of all of the uids in people

let uids = _.pluck(people, "uid");
console.log(uids);

//Find the person with the highest uid
let maxUid = _.max(people, function(p) {
  return p.uid;
});
...
//Return an object that says how many even numbers and how many odd numbers there are in numbers
let count = _.countBy(numbers, function(num) {
...
let sample3 = _.sample(numbers, 3);
console.log(sample3)
