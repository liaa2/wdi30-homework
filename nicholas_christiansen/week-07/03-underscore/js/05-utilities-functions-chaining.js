...
// Append a paragraph tag to the body for every person in people. It should
// end up looking something like this - <p> Hello A, you don't look a day
// over 20 </p>

const initialize = _.once(function () {
  _.each(people, function (p) {
    var node = document.createElement("p");
    var textnode = document.createTextNode(`Hello ${ p.username }, you don't look a day over ${ p.age }`);
...
// The Final Exercise
// This is really, really difficult, and we don't really expect it to get done.
// But worth having a think about!
//
...
const unzipped = _.map(startingData, function (x) {
  return _.unzip(x)
});

console.log(unzipped);

const objected = _.map(unzipped, function (x) {
  return _.object(x)
});
...
const inverted = _.map(objected, function (x) {
  return _.invert(x)
});

console.log(inverted);

const changed = _.each(inverted, function (x) {
  x.born -= 100
...
// _.each(startingData, function (x) {
//
// })
