...
const validateCreditCard = function (a){
  let regNumber = /\d+/; //regular expression of number
  let regString =/[a-zA-Z]+/; // regular expression of letter

  if ( regNumber.test( a ) && regString.test( a)) {
    return "Invalid";
  } else if ( a.toString().length === 16 ) {
    return "Valid";
  }
};
...
and what are some numbers that pass or fail? Ideas: check expiration date! Check
out the Luhn Algorithm for inspiration.*/
