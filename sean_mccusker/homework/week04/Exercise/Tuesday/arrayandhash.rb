...
h.merge( { :four => 4 } ) # Alternatively, direct assignment as above.
...
users[ "Sean" ] = { :twitter => "seando49", :favourite_numbers => [ 4, 13, 17, 19, 12 ] }
...
p users[ "Jonathan" ][ :favorite_numbers ] & users[ "Erik" ][ :favorite_numbers ] & users[ "Anil" ][ :favorite_numbers ] & users[ "Sean" ][ :favorite_numbers ]
# Fancy version 1: users.map { | name, user | user[ :favorite_numbers ] }.inject( &:& )
# More explicit fancy version:
# users.map { | name, user | user[ :favorite_numbers ] }.inject { |memo, n| memo & n }
# 1. How would you return an array containing all users' favorite numbers, sorted, and excluding duplicates?
p users.map { |name, user| user[ :favorite_numbers ] }.flatten.sort.uniq
