...
post '/beers/' do
  query = "INSERT INTO beers (name, brewer, image) VALUES ('#{params[:name]}', '#{params[:brewer]}', '#{params[:image]}')"
  query_db query
...
get '/beers/:id' do
  ...
  @beer = @beer.first # Pluck a single beer from the results array.
  erb :beers_show
end
...
post '/beers/:id' do
  query = "UPDATE beers SET name='#{params[:name]}', brewer='#{params[:brewer]}', image='#{params[:image]}' WHERE id=#{ params[:id] }"
  query_db query
