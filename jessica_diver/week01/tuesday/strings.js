...
const mixUp = function(string1, string2) {
  const start1 = string1.slice(0,2);
  const end1 = string2.slice(2);
  const start2 = string2.slice(0,2);
  const end2 = string1.slice(2);
  const result = `${start1}${end1} ${start2}${end2}`;
  return result;
};
...
const verbing = function (string) {
  if (string.length > 3 && indexOf('ing') < 4){
  return (`${string}ly`);
  }
  else (string.indexOf('ing') > 3 && indexOf('ing')){
  return (`${string}`ly);
  }
